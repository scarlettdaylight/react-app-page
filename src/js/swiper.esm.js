...
const Browser = (function Browser() {
  ...
  return {
    ...
    ieTouch:
      (win.navigator.msPointerEnabled && win.navigator.msMaxTouchPoints > 1) ||
...
var loopCreate = function() {
  ...
  slides.each((index$$1, el) => {
    ...
    if (
      ...
      index$$1 >= slides.length - swiper.loopedSlides
    )
...
var onTouchMove = function(event) {
  ...
  if (data$$1.isTouchEvent && doc.activeElement) {
    if (
      ...
      $(e.target).is(data$$1.formElements)
    ) {
      data$$1.isMoved = true;
      swiper.allowClick = false;
      return;
  ...
  if (typeof data$$1.isScrolling === 'undefined') {
    ...
    if (
      ...
      touchAngle =
        ...
        180 /
        Math.PI;
      data$$1.isScrolling = swiper.isHorizontal()
  ...
  if (diff > 0 && data$$1.currentTranslate > swiper.minTranslate()) {
    ...
    if (params.resistance)
      data$$1.currentTranslate =
        swiper.minTranslate() -
        1 +
        (-swiper.minTranslate() + data$$1.startTranslate + diff) **
          resistanceRatio;
    ...
    if (params.resistance)
      data$$1.currentTranslate =
        swiper.maxTranslate() +
        1 -
        (swiper.maxTranslate() - data$$1.startTranslate - diff) **
          resistanceRatio;
  }
    ...
    const $slideEl = params.renderSlide
      ...
      : $(
          `<div class="${swiper.params
            .slideClass}" data-swiper-slide-index="${index$$1}">${slide}</div>`
        );
...
const Mousewheel = {
  ...
  handle(event) {
    ...
    if (params.forceToAxis) {
      ...
      delta =
        Math.abs(data$$1.pixelX) > Math.abs(data$$1.pixelY)
          ? -data$$1.pixelX * rtlFactor
          : -data$$1.pixelY;
    }
...
const Pagination = {
  update() {
    ...
    if (
      ...
      bullets.removeClass(
        `${params.bulletActiveClass} ${params.bulletActiveClass}-next ${params.bulletActiveClass}-next-next ${params.bulletActiveClass}-prev ${params.bulletActiveClass}-prev-prev`
      );
      if ($el.length > 1) {
  ...
  loadInSlide(index$$1, loadInDuplicate = true) {
    ...
    const $slideEl = isVirtual
      ? swiper.$wrapperEl.children(
          `.${swiper.params.slideClass}[data-swiper-slide-index="${index$$1}"]`
        )
          ...
          if (swiper.params.loop && loadInDuplicate) {
            ...
            if ($slideEl.hasClass(swiper.params.slideDuplicateClass)) {
              const originalSlide = swiper.$wrapperEl.children(
                `[data-swiper-slide-index="${slideOriginalIndex}"]:not(.${swiper
              ...
              const duplicatedSlide = swiper.$wrapperEl.children(
                `.${swiper.params
                  .slideDuplicateClass}[data-swiper-slide-index="${slideOriginalIndex}"]`
              );
  ...
  load() {
    ...
    function slideExist(index$$1) {
      if (isVirtual) {
        if (
          $wrapperEl.children(
            `.${swiperParams.slideClass}[data-swiper-slide-index="${index$$1}"]`
          ).length
        ) {
          return true;
        }
...
var a11y$1 = {
  ...
  create() {
    ...
    Utils.extend(swiper, {
      a11y: {
        liveRegion: $(
          `<span class="${swiper.params.a11y
            .notificationClass}" aria-live="assertive" aria-atomic="true"></span>`
        )
      }
    });
...
const Cube = {
  setTranslate() {
    ...
    for (let i = 0; i < slides.length; i += 1) {
      ...
      const transform$$1 = `rotateX(${isHorizontal
        ? 0
        : -slideAngle}deg) rotateY(${isHorizontal
        ? slideAngle
        : 0}deg) translate3d(${tx}px, ${ty}px, ${tz}px)`;
    ...
    $wrapperEl.transform(
      `translate3d(0px,0,${zFactor}px) rotateX(${swiper.isHorizontal()
        ? 0
        : wrapperRotate}deg) rotateY(${swiper.isHorizontal()
        ? -wrapperRotate
        : 0}deg)`
    );
  },
  ...
  setTranslate() {
    ...
    for (let i = 0; i < slides.length; i += 1) {
      ...
      if (swiper.params.flipEffect.slideShadows) {
        ...
        if (shadowBefore.length === 0) {
          shadowBefore = $(
            `<div class="swiper-slide-shadow-${swiper.isHorizontal()
              ? 'left'
              : 'top'}"></div>`
          );
          $slideEl.append(shadowBefore);
...
// Swiper Class
// Core Modules
Swiper.components = [Device$2, Support$2, Browser$2, Resize, Observer$1];
